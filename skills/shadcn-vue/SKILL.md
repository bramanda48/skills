...
- **Default icon framework is Iconify.** Render via `<Icon icon="prefix:name" />` from `@iconify/vue`. Don't import from per-library Vue packages (`@lucide/vue`, etc.) in code you write.
- **Icons in `Button` use `data-icon`.** `data-icon="inline-start"` or `data-icon="inline-end"` on the `<Icon>`.
- **No sizing classes on icons inside shadcn components.** Components size icons via CSS. Standalone `<Icon>` (not wrapped) still needs explicit sizing.
- **Pass icon names as strings, not component objects.** `<Icon :icon="name" />` where `name` is `"lucide:check"`, not a component lookup.
- **Swap CLI component icons to Iconify.** Components added via `add` import from the project's `iconLibrary` (lucide/tabler/etc.); rewrite those imports to `@iconify/vue` afterward. Bundle sets offline with `@iconify-json/*` + `addCollection`.
...
<!-- Icons: Iconify <Icon> with data-icon, no sizing classes. -->
<Button>
...
- **`iconLibrary`** → CLI-generated components import from this library (`lucide` → `@lucide/vue`, `tabler` → `@tabler/icons-vue`, etc.). Valid values: `lucide`, `tabler`, `hugeicons`, `phosphor`, `remixicon`. There is no `iconify` option — after adding a component, swap its icon imports to `@iconify/vue` (see [icons.md](./rules/icons.md)).
...
7. **Review added components** — After adding a component or block from any registry, **always read the added files and verify they are correct**. Check for missing sub-components (e.g. `SelectItem` without `SelectGroup`), missing imports, incorrect composition, or violations of the [Critical Rules](#critical-rules). Also **swap any icon imports to Iconify** — CLI-generated and third-party registry components import from a per-library package (e.g. `@lucide/vue`, `@tabler/icons-vue`) based on the project's `iconLibrary`; rewrite those to `@iconify/vue` `<Icon icon="prefix:name" />` (see [icons.md](./rules/icons.md)). Fix all issues before moving on.
...
- [rules/icons.md](./rules/icons.md) — Iconify default, data-icon, icon sizing, passing icon names as strings, CLI swap, offline bundling
